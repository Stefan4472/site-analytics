...
def test_count_users_all_time():
    query = Query(
        QueryOn.People, CountWhat.Users, GroupWhat.Nothing, QueryResolution.All
    )
    expected_sql = (
...
def test_count_users_by_day():
    query = Query(
        QueryOn.People, CountWhat.Users, GroupWhat.Nothing, QueryResolution.Day
    )
    expected_sql = (
        "SELECT COUNT(DISTINCT users.id) AS cnt, "
...
def test_count_users_by_week():
    query = Query(
        QueryOn.People, CountWhat.Users, GroupWhat.Nothing, QueryResolution.Week
    )
    expected_sql = (
        "SELECT COUNT(DISTINCT users.id) AS cnt, "
        "EXTRACT(YEAR FROM views.timestamp) AS year, "
...
def test_count_users_by_month():
    query = Query(
        QueryOn.People, CountWhat.Users, GroupWhat.Nothing, QueryResolution.Month
    )
    expected_sql = (
        "SELECT COUNT(DISTINCT users.id) AS cnt, "
        "EXTRACT(YEAR FROM views.timestamp) AS year, "
...
def test_count_users_by_year():
    query = Query(
        QueryOn.People, CountWhat.Users, GroupWhat.Nothing, QueryResolution.Year
    )
    expected_sql = (
        "SELECT COUNT(DISTINCT users.id) AS cnt, "
        "EXTRACT(YEAR FROM views.timestamp) AS year "
...
def test_country_views_by_week():
    query = Query(
        QueryOn.Bots, CountWhat.Views, GroupWhat.Country, QueryResolution.Week
    )
    expected_sql = (
        "SELECT COUNT(*) AS cnt, users.country, "
        "EXTRACT(YEAR FROM views.timestamp) AS year, "
...
def test_country_views_by_month():
    query = Query(
        QueryOn.Bots, CountWhat.Views, GroupWhat.Country, QueryResolution.Month
    )
    expected_sql = (
        "SELECT COUNT(*) AS cnt, users.country, "
        "EXTRACT(YEAR FROM views.timestamp) AS year, "
...
def test_country_views_by_year():
    query = Query(
        QueryOn.Bots, CountWhat.Views, GroupWhat.Country, QueryResolution.Year
    )
    expected_sql = (
        "SELECT COUNT(*) AS cnt, users.country, "
        "EXTRACT(YEAR FROM views.timestamp) AS year "
